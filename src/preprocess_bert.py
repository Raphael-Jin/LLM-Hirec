...
def read_news(path,filenames):
    ...
    for line in lines:
        splited = line.strip('\n').split('\t')
        doc_id,vert,subvert,title, abstraction= splited[0:5]
        ...
        abstraction = word_tokenize(abstraction)
        title=word_tokenize(title)
        news[doc_id]=[vert,subvert,title,abstraction]
        for word in title:
            word = word.lower() 
...
def get_doc_input(news,news_index,category,subcategory,word_dict):
    ...
    news_abs = np.zeros((news_num,MAX_SENTENCE),dtype='int32')
    news_vert=np.zeros((news_num,),dtype='int32')
    news_subvert=np.zeros((news_num,),dtype='int32')
    for key in news:    
        vert,subvert,title,abstraction=news[key]
